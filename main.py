...
def cmd_stats(args):
    ...
    honny = HonnyMedia()
    stats = honny.get_stats(days=args.days)
...
def cmd_gallery(args):
    ...
    honny = HonnyMedia()
    images = honny.get_gallery(workflow=args.type, limit=args.limit)
...
def cmd_cleanup(args):
    ...
    honny = HonnyMedia()
    result = honny.cleanup(days=args.days)
...
def cmd_set_default_ref(args):
    """设置默认参考图"""
    from honny_media import HonnyMedia
    
    honny = HonnyMedia()
    url = honny.set_default_reference(args.path)
...
def cmd_show_default_ref(args):
    """显示当前默认参考图"""
    from honny_media import HonnyMedia
    
    honny = HonnyMedia()
    ref = honny.get_default_reference()
    
    if ref:
        print(f"\n📷 当前默认参考图:")
        print(f"  ID: {ref['id']}")
        print(f"  路径: {ref['file_path']}")
        print(f"  URL: {ref['remote_url']}")
        print(f"  上传时间: {ref['uploaded_at']}")
    else:
        print("\n⚠️ 未设置默认参考图")
...
def main():
    ...
    # set-default-ref 命令
    set_ref_parser = subparsers.add_parser('set-default-ref', help='设置默认参考图')
    set_ref_parser.add_argument('path', help='参考图路径')
    set_ref_parser.set_defaults(func=cmd_set_default_ref)
    
    # show-default-ref 命令
    show_ref_parser = subparsers.add_parser('show-default-ref', help='显示当前默认参考图')
    show_ref_parser.set_defaults(func=cmd_show_default_ref)
